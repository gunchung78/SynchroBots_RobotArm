...
# READ_METHOD_NODE_ID = "ns=2;s=read_arm_go_move" # 미사용
...
async def send_mission_state(status: str):
    """미션 상태(arm_mission_success/failure)를 서버에 송신합니다. (WRITE_OBJECT_NODE_ID, READ_METHOD_NODE_ID 사용)"""
    ...
    try:
        async with Client(OPCUA_WRITE_URL) as client:
            ...
            print(f"\n[OPC UA WRITE - MISSION_STATE] call_method(status='{status}') (Method: {READ_METHOD_NODE_ID})")
...
async def arm_subscriber():
    ...
    try:
        async with Client(OPCUA_READ_URL) as client:
            ...
            # READ_OBJECT_NODE_ID/READ_METHOD_NODE_ID가 아닌 CMD_NODE_PATH의 Variable을 구독합니다.
