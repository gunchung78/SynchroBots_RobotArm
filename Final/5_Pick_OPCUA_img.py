...
# ===============================================
# 📌 AI 모델 및 상수 설정 (1_Pick.py에서 가져옴)
# ===============================================
CLASS_NAMES = ["ESP32", "L298N(Motor)", "MB102(Power)"]
NUM_CLASSES = len(CLASS_NAMES)
# DEVICE는 OPC UA와 MyCobot이 비동기/CPU 위주 작업이므로 잠시 CPU로 고정하거나,
# 실제 환경에 맞춰 조정해야 합니다. GPU를 쓰려면 asyncio 환경에서 주의가 필요합니다.
...
# --- OPC UA 수신/송신 설정 (3_read_Pick_OPCUA.py에서 가져옴) ---
# OPCUA_READ_URL = "opc.tcp://172.30.1.61:0630/freeopcua/server/"
# OPCUA_WRITE_URL = "opc.tcp://172.30.1.61:0630/freeopcua/server/"
OPCUA_READ_URL = "opc.tcp://127.0.0.1:4840/freeopcua/server/" # ⚠️ LOCAL_SERVER_URL과 동일하게 수정
OPCUA_WRITE_URL = "opc.tcp://127.0.0.1:4840/freeopcua/server/" # ⚠️ LOCAL_SERVER_URL과 동일하게 수정
OBJECT_NODE_ID = "ns=2;i=4"
METHOD_NODE_ID = "ns=2;i=26"
CMD_NODE_PATH = [
    "0:Objects",
    "2:ARM",
    "2:read_arm_go_move"
...
# ===============================================
# 🛠️ 로봇 및 비전 제어 함수 (1_Pick.py에서 가져옴)
# ===============================================
...
def find_object_center(frame):
    ...
    # 🚨 주의: 비동기 환경에서는 imshow를 사용하면 안 됩니다. 디버깅용으로만 사용!
...
async def pick_and_place_vision_guided(mc, cap, ai_model):
    ...
    # logger.info(f"🔍 픽셀 오차: {error:.2f} 픽셀. 로봇 보정 이동량: (X: {delta_X:.2f}mm, Y: {delta_Y:.2f}mm)")
    ...
    # 다음 로직: 분류 결과에 따라 Place (배치) 작업 진행 (미포함)
    
    return True, predicted_class, confidence, target_pose

# ===============================================
# 🌐 OPC UA 통신 함수 (3_read/4_write_Pick_OPCUA.py에서 가져옴, 비전 결과 전송 추가)
# ===============================================

async def send_mission_state(status: str):
    """미션 상태(arm_mission_success/failure)를 서버에 송신합니다."""
    ...
    try:
        async with Client(OPCUA_WRITE_URL) as client:
            ...
            logger.info(f"OPC UA 송신 완료. ResultCode: {result_code}")
...
async def send_vision_result(module_type: str, confidence: float, pick_coord: list, image_to_send: np.ndarray = None):
    """
    분류 및 픽업 결과를 JSON 형태로 묶어 OPC UA 서버에 송신합니다.
    """
    global OPCUA_WRITE_URL, OBJECT_NODE_ID, METHOD_NODE_ID
    ...
    if image_to_send is not None:
        try:
            ...
            # JPEG는 손실 압축이므로 데이터 크기를 크게 줄여줍니다.
    ...
    # ⚠️ 사용자가 요청한 JSON 구조: module_type, classification_confidence, pick_coord, pick_coord_confidence
    vision_result = {
    ...
    try:
        async with Client(OPCUA_WRITE_URL) as client:
            ...
            print(f"\n[OPC UA WRITE] write_vision_result(Module: {module_type}, Conf: {confidence*100:.2f}%)")
            result_code, result_message = await obj.call_method(
                method_node.nodeid,
                json_str
            )
...
async def arm_subscriber():
    ...
    try:
        async with Client(OPCUA_READ_URL) as client:
            ...
            # SubHandler 초기화 시 MyCobot, Camera, AI 모델 인스턴스 모두 전달
            handler = SubHandler(mc, cap, ai_model)
            sub = await client.create_subscription(100, handler)
            
            cmd_node = await client.nodes.root.get_child(CMD_NODE_PATH)
            
            await sub.subscribe_data_change(cmd_node)
            logger.info(f"노드 '{CMD_NODE_PATH[-1]}' 구독 시작. 데이터 수신 대기 중...")
